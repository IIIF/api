---
title: IIIF API Annex Documents
layout: spec
toc: false
hero:
  image: ''
---


## API Annex Documents

- [Open Annotation ImageApiSelector]({{ site.api_url | absolute_url }}/annex/openannotation/)
- [Description of REST usage with IIIF APIs]({{ site.api_url | absolute_url }}/annex/rest/)
- [Known External Services]({{ site.api_url | absolute_url }}/annex/services/)

## API Implementation Notes

- [Apache HTTP Server Implementation Notes]({{ site.api_url | absolute_url }}/annex/notes/apache)
- [JSON-LD Implementation Notes]({{ site.api_url | absolute_url }}/annex/notes/jsonld/)
- [Note on Image Rotation Algorithm]({{ site.api_url | absolute_url }}/annex/notes/rotation/)
- [Note on Semantic Versioning]({{ site.api_url | absolute_url }}/annex/notes/semver/)
- [Disclaimer of Liability]({{ site.api_url | absolute_url }}/annex/notes/disclaimer/)
- [IIIF Design Patterns]({{ site.api_url | absolute_url }}/annex/notes/design_patterns/)
