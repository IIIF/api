...
`navPlace` is intended to connect IIIF web resources with geographic areas. These areas _SHOULD_ be bounded discrete regions of the map akin to extents. `navPlace` is not meant for geographic layouts at the township, city, country or continental level. Such descriptive datasets rely heavily on the client map viewer API for displaying connected metadata directly to the user. The metadata that is connected with the geographic area(s) _SHOULD_ come from the IIIF resource itself and _MAY_ be repeated in GeoJSON `properties` to leverage the display functionality of web maps. It is up to the implementer to determine how to best leverage their chosen web map viewer to display desired metadata along with the rendered shapes.
