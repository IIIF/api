---
title: "IIIF Content State API 0.4"
title_override: "IIIF Content State API 0.4"
...
## 5. Media Type of Content State Documents

The format for all content state responses is JSON, as described above. It is good practice for all resources with an HTTP(S) URI to provide their description when the URI is dereferenced.
...
If the request does not include an `Accept` header, the HTTP `Content-Type` header of the response _SHOULD_ have the value `application/ld+json` (JSON-LD) with the `profile` parameter given as the [Presentation API][prezi3] context document: `http://iiif.io/api/presentation/3/context.json`.
