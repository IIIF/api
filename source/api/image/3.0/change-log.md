...
#### 1.3.1. Support for `square` required at `level1` and `level2` compliance
...
#### 1.3.2. Support for `pct:x` size no longer required at `level1` compliance

The `pct:x` size form is no longer required at `level1` compliance. This is now consistent with `pct:x,y,w,h` for region as both `pct:` parameter forms are `level2` features. See issue [#478](https://github.com/IIIF/api/issues/478).
...
#### 3.9. Implementation notes moved to a separate document
...
#### 3.10. Implementation with static web resources
...
#### 3.11. Added note encouraging adoption of HTTP/2
