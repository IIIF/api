...
| `thumbnail-nav` | Valid only for Range. Ranges with this `behavior` _MAY_ be used by the client to present an alternative navigation or overview based on thumbnails, such as regular keyframes along a timeline for a video. Clients _SHOULD NOT_ use them to generate a conventional table of contents. Child Ranges within a Range with this `behavior` _MUST_ have a suitable `thumbnail` property. |
