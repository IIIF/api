...
If there is no Range that has the `behavior` "sequence", and the Manifest does not have the `behavior` "unordered", then the client _SHOULD_ treat the order of the Canvases in the Manifest's `items` array as the default order. If any Range has the `behavior` value "sequence", then the viewer _MUST_ instead use this Range for the ordering. If there is more than one Range with the `behavior` value "sequence", for example a second Range to represent an alternative ordering of the pages of a manuscript, the first Range _SHOULD_ be used as the default and the others _SHOULD_ be able to be selected.  Ranges with the `behavior` value "sequence" _MUST_ be directly within the `structures` property of the Manifest, and _MUST NOT_ be embedded or referenced within other Ranges.
