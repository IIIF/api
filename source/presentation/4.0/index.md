...
__Previous Version:__ [3.0.0][prezi30]
...
An ordered list of Canvases, Scenes, and/or further Ranges. Ranges allow Canvases, Scenes, or parts thereof, to be grouped together in some way. This could be for content-based reasons, such as might be described in a table of contents or the set of scenes in a play. Equally, physical features might be important such as page gatherings in an early book, or when recorded music is split across different physical carriers such as two CDs.
 ...
 * A Scene _SHOULD_ have the `label` property with at least one entry.<br/>
   Clients _SHOULD_ render `label` on a Scene, and _MAY_ generate a `label` for Canvases that do not have them.
 ...
 * A Scene _MAY_ have the `metadata` property with at least one item.<br/>
   Clients _SHOULD_ render `metadata` on a Scene.
 ...
 * A Scene _MAY_ have the `summary` property with at least one entry.<br/>
   Clients _SHOULD_ render `summary` on a Scene.
 ...
 * A Scene _MAY_ have the `thumbnail` property with at least one item. A Scene _SHOULD_ have the `thumbnail` property if there are multiple resources that make up the view.<br/>
   Clients _SHOULD_ render `thumbnail` on a Scene.
 ...
 * A Scene _MAY_ have the `navDate` property.<br/>
   Clients _MAY_ render `navDate` on a Canvas.
...
**Embed the navPlace extension here**

##### placeholder

A single Canvas or Scene that provides additional content for use before the main content of the resource that has the `placeholder` property is rendered, or as an advertisement or stand-in for that content. Examples include images, text and sound standing in for video content before the user initiates playback; or a film poster to attract user attention. The content provided by `placeholder` differs from a thumbnail: a client might use `thumbnail` to summarize and navigate multiple resources, then show content from `placeholder` as part of the initial presentation of a single resource. A placeholder Canvas is likely to have different dimensions to those of the Canvas(es) of the resource that has the `placeholder` property.

Clients _MAY_ display the content of a linked placeholder Canvas or Scene when presenting the resource. When more than one such placeholder Canvas or Scene is available, for example if `placeholder` is provided for the currently selected Range and the current Manifest, the client _SHOULD_ pick the one most specific to the content. Publishers _SHOULD NOT_ assume that the `placeholder` property will be processed by all clients. Clients _SHOULD_ take care to avoid conflicts between time-based media in the rendered placeholder Canvas or Scene and the content of the resource that has the `placeholder` property.

The value _MUST_ be a JSON object with the `id` and `type` properties, and _MAY_ have other properties of Canvases. The value of `type` _MUST_ be the string `Canvas` or `Scene`. The object _MUST NOT_ have the `placeholder` property, nor the `accompanyingCanvas` property.

  * A Collection _MAY_ have the `placeholder` property.<br/>
    Clients _MAY_ render `placeholder` on a Collection.
  * A Manifest _MAY_ have the `placeholder` property.<br/>
    Clients _MAY_ render `placeholder` on a Manifest.
  * A Canvas _MAY_ have the `placeholder` property.<br/>
    Clients _MAY_ render `placeholder` on a Canvas.
  * A Scene _MAY_ have the `placeholder` property.<br/>
    Clients _MAY_ render `placeholder` on a Scene.
  * A Range _MAY_ have the `placeholder` property.<br/>
    Clients _MAY_ render `placeholder` on a Range.
  * Other types of resource _MUST NOT_ have the `placeholder` property.<br/>
    Clients _SHOULD_ ignore `placeholder` on other types of resource.
...
Clients _MAY_ display the content of an accompanying Canvas when presenting the resource. As with `placeholder` above, when more than one accompanying Canvas is available, the client _SHOULD_ pick the one most specific to the content. Publishers _SHOULD NOT_ assume that the accompanying Canvas will be processed by all clients. Clients _SHOULD_ take care to avoid conflicts between time-based media in the accompanying Canvas and the content of the resource that has the `accompanyingCanvas` property.

The value _MUST_ be a JSON object with the `id` and `type` properties, and _MAY_ have other properties of Canvases. The value of `type` _MUST_ be the string `Canvas`. The object _MUST NOT_ have the `placeholder` property, nor the `accompanyingCanvas` property.
 ...
 * A Scene _MAY_ have the `accompanyingCanvas` property.<br/>
   Clients _MAY_ render `accompanyingCanvas` on a Scene.
 ...
 * Other types of resource, including Scenes, _MUST NOT_ have the `height` property.<br/>
 ...
 * Other types of resource, including Scenes, _MUST NOT_ have the `width` property.<br/>
 ...
 * A Scene _MAY_ have the `duration` property.<br/>
   Clients _MUST_ process `duration` on a Scene.
...
* A Scene _SHOULD_ have the `ambientLighting` property.
  Clients _MUST_ process the `ambientLighting` property on a Scene.
* Other resource types _MUST NOT_ have the `ambientLighting` property.
  Clients _SHOULD NOT_ process the `ambientLighting` property on other resource types.
...
* Other resources _MUST NOT_ have the `services` property.<br/>
  Clients _SHOULD NOT_ process `services` on other resources.
 ...
 * A Scene _SHOULD_ have the `items` property with at least one item. Each item _MUST_ be an Annotation Page.<br/>
   Clients _MUST_ process `items` on a Scene.
 ...
 * A Scene _MAY_ have the `annotations` property with at least one item.<br/>
   Clients _SHOULD_ process `annotations` on a Scene.
