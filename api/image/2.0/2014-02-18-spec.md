...
If the resulting height or width is zero, then the server _should_{: .rfc} return a 400 (bad request) status code.
...
A format value that is unsupported _should_{: .rfc} result in a 400 status code.
