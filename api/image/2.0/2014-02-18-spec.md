...
The syntax for the response is [JSON-LD][37]. The content-type of the response MUST be either "application/json" (regular JSON), or "application/ld+json" (JSON-LD).  If the client explicitly wants the JSON-LD content-type, then it must specify this in an Accept header, otherwise the server must return the regular JSON content-type.
...
If a property is not present in either the profile document or the `supports` property, then a client MUST assume that the feature is not supported. 
